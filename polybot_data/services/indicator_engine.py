...
Arguments:
    prior_candles:      list of CandleRecord (chronological)
    candle_open:        float, BTC open price of current candle
    snapshots_so_far:   list of IndicatorSnapshot seen so far in this candle
...
def _snap_up_mid(s: IndicatorSnapshot) -> float | None:
    if s.up_bids and s.up_asks:
        return (s.up_bids[0][0] + s.up_asks[0][0]) / 2
    return None


def _snap_down_mid(s: IndicatorSnapshot) -> float | None:
...
def prior_return(prior_candles: list[CandleRecord]) -> float | None:
...
def consecutive_streak(prior_candles: list[CandleRecord]) -> int | None:
...
def streak_magnitude(prior_candles: list[CandleRecord]) -> float | None:
...
def rolling_volatility(prior_candles: list[CandleRecord], window: int = 6) -> float | None:
...
def candle_momentum(prior_candles: list[CandleRecord], window: int = 6) -> float | None:
...
def ma_crossover(prior_candles: list[CandleRecord]) -> float | None:
...
def trend_consistency(prior_candles: list[CandleRecord], window: int = 10) -> float | None:
...
def reversal_regime(prior_candles: list[CandleRecord], window: int = 4) -> float | None:
...
def rsi(prior_candles: list[CandleRecord], period: int = 14) -> float | None:
...
def bollinger_pct_b(prior_candles: list[CandleRecord], period: int = 20, num_std: float = 2.0) -> float | None:
...
def stochastic_k(prior_candles: list[CandleRecord], period: int = 14) -> float | None:
...
def adx(prior_candles: list[CandleRecord], period: int = 14) -> float | None:
...
def return_autocorrelation(prior_candles: list[CandleRecord], lag: int = 1, window: int = 20) -> float | None:
...
def multi_candle_return_3(prior_candles: list[CandleRecord]) -> float | None:
...
def multi_candle_return_6(prior_candles: list[CandleRecord]) -> float | None:
...
def btc_move_from_open(candle_open: float, snapshots: list[IndicatorSnapshot]) -> float | None:
...
def btc_velocity(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def intra_candle_volatility(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def peak_drawback(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def btc_acceleration(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def btc_direction_consistency(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def intra_candle_skewness(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def intra_candle_kurtosis(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def hurst_exponent(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def market_efficiency_lag(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def up_implied_probability(snapshots: list[IndicatorSnapshot]) -> float | None:
    """UP token midpoint at current snapshot."""
    return _snap_up_mid(snapshots[-1]) if snapshots else None


def up_book_imbalance(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def cross_book_flow(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def up_spread_level(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def token_price_divergence(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def up_risk_reward(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def up_token_velocity(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def down_implied_probability(snapshots: list[IndicatorSnapshot]) -> float | None:
    """DOWN token midpoint at current snapshot."""
    return _snap_down_mid(snapshots[-1]) if snapshots else None


def down_book_imbalance(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def down_token_velocity(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def down_risk_reward(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def rr_spread(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def weighted_mid_price(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def btc_token_correlation(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def liquidity_decay(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def imbalance_momentum(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def spread_compression(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def observation_window(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def current_elapsed(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def time_of_day_sin(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def time_of_day_cos(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def imbalance_shift(snapshots: list[IndicatorSnapshot]) -> float | None:
    """Late imbalance minus early imbalance (last third vs first third)."""
    if len(snapshots) < 9:
        return None
    third = len(snapshots) // 3

    def avg_imb(snap_list: list[IndicatorSnapshot]) -> float | None:
        vals = []
        for s in snap_list:
            if s.up_bids and s.up_asks and s.up_asks[0][1] > 0:
...
def smart_money_signal(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def depth_absorption_rate(candle_open: float, current_price: float, snapshots: list[IndicatorSnapshot]) -> float | None:
...
def regime_score(prior_candles: list[CandleRecord], window: int = 6) -> float | None:
...
def ob_pressure_gradient(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def conviction_score(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def price_path_entropy(snapshots: list[IndicatorSnapshot]) -> float | None:
...
def mean_reversion_signal(prior_candles: list[CandleRecord], lookback: int = 20) -> float | None:
...
def btc_drawdown_from_peak(candle_open: float, snapshots: list[IndicatorSnapshot]) -> float | None:
...
def prior_reversal_rate(prior_candles: list[CandleRecord], window: int = 6) -> float | None:
...
def compute_all(
    ...
    candle_open: float,
    snapshots_so_far: list[IndicatorSnapshot],
) -> dict[str, Any]:
    """Compute all indicators for a single snapshot moment.

    Args:
        prior_candles:    completed CandleRecords before this one (chronological)
        candle_open:      BTC open price of current candle
        snapshots_so_far: IndicatorSnapshots #0..#N in current candle (N = current)
    ...
    return {
        ...
        "depth_absorption_rate": depth_absorption_rate(candle_open, s[-1].btc_price if s else candle_open, s),
