...
class MarketHistoryStore:
    ...
    def __init__(self, db_path: str | Path, iteration: str = "") -> None:
        self._db_path = Path(db_path)
    ...
    def open(self) -> None:
        ...
        self._conn.commit()
        logger.info("MarketHistoryStore opened: %s (iteration=%s)", self._db_path, self._iteration)
    ...
    async def close(self) -> None:
        ...
        self._conn.close()
        self._conn = None
        logger.info("MarketHistoryStore closed")
    ...
    def begin_candle(
        ...
        try:
            ...
            logger.exception("MarketHistoryStore: begin_candle failed")
            return None
    ...
    def queue_snapshot(self, row: MarketSnapshotRow) -> None:
        try:
            ...
            logger.warning("MarketHistoryStore: snapshot queue full, dropping row")

    # --- Background writer task ---

    async def writer_loop(self) -> None:
        logger.info("MarketHistoryStore writer_loop started")
        last_flush = time.monotonic()
        ...
        while True:
            try:
                ...
                await self._flush()
                logger.info("MarketHistoryStore writer_loop stopped")
                return
            except Exception:
                logger.exception("MarketHistoryStore writer_loop error")
                await asyncio.sleep(1.0)
    ...
    async def _flush(self) -> None:
        ...
        try:
            ...
            self._conn.commit()
            if snapshots:
                logger.debug("MarketHistoryStore flushed %d snapshots", len(snapshots))
        except Exception:
            logger.exception("MarketHistoryStore flush error")
