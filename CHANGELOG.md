...
- **Edge-based strategy engine** — Rewrote `notebooks/strategy_engine.py` to use edge-based parameters (edge = max(prob, 1-prob) - ask_price) instead of elapsed/consecutive/confidence parameters. `StrategyConfig` now takes `min_edge` and `max_entries` fields. `StrategyGrid` generates a simple cartesian product (7 edge thresholds x 2 max_entries = 14 configs by default). `run_scaling()` computes per-snapshot edge and fires entries when edge >= threshold with direction lock. `WalkForwardEvaluator` unchanged. Fixed None ask crash in run_scaling (check before edge computation).
### Fixed
- **Strategy overfitting** — Previous strategy evaluation used the exported model (trained on ALL data) to generate predictions, then ran walk-forward on those predictions. This inflated results massively (RF showed 98.2% WR / +1652% — memorized training data). Fixed by training a fresh model on 80% and evaluating strategy on the unseen 20%. Honest results: RF +9.3%, LR +16.6%, XGB +8.5%, DNN +5.7%.
...
- **`datastore`** — Converted from single 822-LOC file to package; split into `rows.py` (3 row dataclasses), `store.py` (DataStore), `market_history.py` (MarketHistoryStore); extracted flush/pragma constants to `constants.py`; made loggers injectable in both stores; populated `__init__.py` with backward-compatible re-exports; 32 new tests covering constants, rows, candle lifecycle, flush, and close; README documenting write pipeline and store architecture
- **`knowledge`** — Converted from single 767-LOC file to package; extracted scorecard computation to `scorecard.py`; extracted 20+ constants to `constants.py`; made logger injectable in `KnowledgeManager`; populated `__init__.py` with backward-compatible re-exports; 38 new tests covering constants, scorecard, observations, cache, session history, and feedback context; README documenting architecture and observation lifecycle
...
- **`simulator`** — Extracted 8 magic numbers to `constants.py`; made loggers injectable in `ExecutionSimulator`, `SimulatedOrderBook`, and `Portfolio`; populated `__init__.py` with re-exports; 43 new tests covering engine slippage/fills, orderbook limit orders, and portfolio PnL/resolution; README documenting execution pipeline and slippage model
...
- **`tasks`** — Extracted 4 pure prompt-context helpers from `ai_decision.py` (1779 LOC) into `tasks/prompt_context.py` (25 tests); extracted 10 decision guard functions into `tasks/decision_guards.py` (36 tests); extracted 5 context-building functions into `tasks/context_builder.py` (18 tests); extracted trade record/decision row builders into `tasks/trade_logger.py` (13 tests); `ai_decision.py` reduced from 1779 to 1111 LOC
