...
class TestVerifyWinner:
    ...
    async def test_no_client_returns_btc_winner(self):
        result = await verify_winner(_market(), "up", None)
        assert result == "up"

    async def test_agreement(self, mock_client):
        mock_client.get_last_trade_price = AsyncMock(side_effect=[0.95, 0.05])
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
            result = await verify_winner(_market(), "up", mock_client)
        assert result == "up"

    async def test_mismatch_uses_polymarket(self, mock_client):
        # BTC says "up" but Polymarket says "down"
        mock_client.get_last_trade_price = AsyncMock(side_effect=[0.05, 0.95])
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
            result = await verify_winner(_market(), "up", mock_client)
        assert result == "down"

    async def test_ambiguous_uses_btc(self, mock_client):
        mock_client.get_last_trade_price = AsyncMock(side_effect=[0.50, 0.50])
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
            result = await verify_winner(_market(), "down", mock_client)
        assert result == "down"

    async def test_api_error_uses_btc(self, mock_client):
        mock_client.get_last_trade_price = AsyncMock(side_effect=RuntimeError("API down"))
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
            result = await verify_winner(_market(), "up", mock_client)
        assert result == "up"
...
class TestResolutionTracker:
    ...
    @pytest.fixture
    def tracker(self, btc_feed, rest_client):
        return ResolutionTracker(btc_feed, rest_client)
    ...
    async def test_resolve_up(self, tracker, rest_client):
        market = _market()
        tracker.record_candle_open(market, 65000.0)
        # Polymarket confirms UP
        rest_client.get_last_trade_price = AsyncMock(side_effect=[0.95, 0.05])
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
        ...
        # Open price consumed (popped)
        assert tracker.get_candle_open("cond_1") is None

    async def test_resolve_down(self, tracker, rest_client):
        market = _market()
        tracker.record_candle_open(market, 65100.0)
        rest_client.get_last_trade_price = AsyncMock(side_effect=[0.05, 0.95])
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
            record = await tracker.resolve(market, 65000.0)
        assert record.winner == "down"

    async def test_resolve_no_open_falls_back_to_feed(self, tracker, btc_feed, rest_client):
        btc_feed.get_price_at = AsyncMock(return_value=65000.0)
        rest_client.get_last_trade_price = AsyncMock(side_effect=[0.95, 0.05])
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
            record = await tracker.resolve(_market(), 65100.0)
        assert record.btc_open == 65000.0
        assert record.winner == "up"
        btc_feed.get_price_at.assert_called_once_with(1000.0)

    async def test_resolve_no_open_no_feed_defaults_to_close(self, tracker, btc_feed, rest_client):
        btc_feed.get_price_at = AsyncMock(return_value=None)
        rest_client.get_last_trade_price = AsyncMock(side_effect=[0.95, 0.05])
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
            record = await tracker.resolve(_market(), 65100.0)
        # open defaults to close → tie → "up"
        assert record.btc_open == 65100.0
        assert record.winner == "up"

    async def test_resolve_returns_resolution_record_fields(self, tracker, rest_client):
        market = _market()
        tracker.record_candle_open(market, 65000.0)
        rest_client.get_last_trade_price = AsyncMock(side_effect=[0.95, 0.05])
        with patch("polybot.resolution.verifier.asyncio.sleep", new_callable=AsyncMock):
