...
class TestProcessorNoConfig:
    ...
    def test_rr_and_best_entry_computed(self):
        processor = IndicatorsProcessor(all_indicators())
        snap = _make_snapshot(up_ask=0.40, down_ask=0.60)
        results = processor.compute(snap)
        rr = results.get("Risk/Reward")
        assert rr is not None
        assert rr.value == pytest.approx(1.5)  # max((1-0.4)/0.4, (1-0.6)/0.6)
        assert results.get_value("Best Entry") == pytest.approx(0.40)
...
class TestProcessorIndicatorValues:
    """Verify specific indicator values via get / get_value."""
    ...
    def test_btc_move_from_open(self):
        ...
        assert results.get_value("BTC Move From Open") == pytest.approx(100.0)

    def test_btc_move_none_when_no_candle_open(self):
        processor = IndicatorsProcessor(all_indicators())
        snap = _make_snapshot()
        results = processor.compute(snap)
        assert results.get("BTC Move From Open") is None
    ...
    def test_best_entry(self):
        ...
        results = processor.compute(snap)
        assert results.get_value("Best Entry") == pytest.approx(0.30)
