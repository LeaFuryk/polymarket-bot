...
const MODE_BADGE: Record<
  string,
  { label: string; variant: "green" | "blue" | "amber" }
> = {
...
function IterationCard({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-white/5 bg-[#131720]">
      <button
        onClick={onToggle}
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-4">
          ...
          {iter.live_trading && (
            <span
              className={`font-mono text-[11px] ${pnlColor(-iter.live_trading.execution_cost)}`}
            >
              {iter.live_trading.execution_cost >= 0 ? "+" : ""}
              {formatCurrency(iter.live_trading.execution_cost, 2)} cost
            </span>
          )}
      ...
      {expanded && (
        <div className="space-y-4 border-t border-white/5 bg-[#0d1017] p-4">
          {/* Core stats */}
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
            ...
            {iter.total_fees !== undefined && (
              <MetricCard label="Fees" value={formatUsd(iter.total_fees, 2)} />
            )}
            {iter.ai_cost !== undefined && (
              <MetricCard label="AI Cost" value={formatUsd(iter.ai_cost, 2)} />
            )}
          </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <MetricCard
                  ...
                  value={
                    <span
                      className={pnlColor(iter.live_trading.shadow_paper_pnl)}
                    >
                ...
                <MetricCard
                  ...
                  value={
                    <span
                      className={pnlColor(-iter.live_trading.execution_cost)}
                    >
          ...
          {iter.trade_analysis && (
            <div>
              ...
              <div className="mt-2 flex items-center gap-1 text-[11px]">
                <span className="mr-1 text-zinc-500">Entries:</span>
          ...
          {iter.exit_analysis && iter.exit_analysis.total_exits > 0 && (
            <div className="flex items-center gap-4 text-xs">
              ...
              <span className="text-zinc-600">
                ({iter.exit_analysis.good_exits}/
                {iter.exit_analysis.total_exits})
              </span>
            </div>
          )}
            ...
            <div>
              ...
              <div className="max-h-48 overflow-y-auto">
                <table className="w-full text-[11px]">
                  <thead>
                    <tr className="border-b border-white/5 tracking-wider text-zinc-600 uppercase">
                      <th className="pr-2 pb-1 text-left">Candle</th>
                      <th className="pr-2 pb-1 text-right">BTC Move</th>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {iter.resolutions_detail.map((r, i) => (
                      <tr key={`${r.slug}-${i}`}>
                        <td className="max-w-[200px] truncate py-1 pr-2 font-mono text-zinc-400">
                        ...
                        <td
                          className={`py-1 text-right font-mono ${pnlColor(r.pnl)}`}
                        >
...
export default function HistoryPage() {
  ...
  >(null);
  const [modeFilter, setModeFilter] = useState<Set<string>>(
    new Set(["paper", "live", "dry_run"]),
  );
  ...
  const sorted = [...iterations].sort((a, b) => b.label.localeCompare(a.label));
  ...
  // Mode breakdown
  const paperCount = filtered.filter(
    (i) => (i.trading_mode ?? "paper") === "paper",
  ).length;
  const liveCount = filtered.filter((i) => i.trading_mode === "live").length;
  const dryRunCount = filtered.filter(
    (i) => i.trading_mode === "dry_run",
  ).length;
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-4 text-xs text-zinc-400">
          <span>
            {paperCount > 0 && (
              <span className="text-green-400">{paperCount} paper</span>
            )}
            {liveCount > 0 && (
              <>
                {paperCount > 0 && " / "}
            ...
            {dryRunCount > 0 && (
              <>
                ...
                <span className="text-amber-400">{dryRunCount} dry-run</span>
              </>
            )}
          </span>
      ...
      {sorted.length > 0 && (
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
          ...
          <div className="flex items-center gap-2">
            <span className="mr-1 text-[11px] tracking-wider text-zinc-600 uppercase">
              Mode
            </span>
                ...
                <button
                  ...
                  className={`rounded-full border px-2.5 py-1 text-[11px] font-medium transition-colors ${
                    active
                      ...
                      : "border-transparent bg-white/5 text-zinc-500 hover:bg-white/10"
                  }`}
                >
                  {cfg.label}
                </button>
            ...
            {[
              {
                value: "all" as const,
                label: "All",
                cls: "bg-white/15 text-zinc-200 border-white/20",
              },
              {
      ...
      {sorted.length === 0 ? (
        <div className="py-12 text-center text-sm text-zinc-600">
          No iterations found. Run the bot and archive sessions to generate
          iteration data.
        </div>
      ) : filtered.length === 0 ? (
        <div className="py-12 text-center text-sm text-zinc-600">
